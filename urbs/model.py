...
def create_model(data, dt=1, timesteps=None, objective='cost',
    ...
    m.com_demand_tuples = pyomo.Set(
        within=m.stf * m.sit * m.com * m.com_type,
        initialize=[(stf, sit, com, com_type) for (stf, sit, com, com_type) in m.commodity_dict["price"].keys() if
                    com_type == 'Demand'],
        doc='Combinations of defined commodities, e.g. (2018,Mid,Elec,Demand)')
    ...
    # fusion
    m.demand_fusion = pyomo.Var(
        m.tm, m.com_demand_tuples,
        within=pyomo.NonNegativeReals,
        doc='Additional demand (MW) from continous losses of nuclear fusion reactors per timestep')
    ...
    m.output_pro = pyomo.Var(
        m.pro_tuples,
        within=pyomo.NonNegativeReals,
        doc = 'output ratio of processes'
    )
    ...
    m.def_process_output_ratio_range = pyomo.Constraint(
        m.pro_tuples,
        rule=def_process_output_ratio_range,
        doc='ratio_low <= portion <= ratio_up'
    )
    m.def_process_output_portion = pyomo.Constraint(
        m.tm, (m.pro_output_tuples - m.pro_partial_output_tuples -
               m.pro_timevar_output_tuples),
        rule=def_process_output_portion,
        doc='process output = portion * demand'
    )
    ...
    # fusion heat
    m.def_continious_losses = pyomo.Constraint(
        m.tm, m.pro_tuples,
        rule=def_continious_losses_rule,
        doc='process heat demand from continious losses of nuclear fusion reactors contingent on process capacity')
...
def res_vertex_rule(m, tm, stf, sit, com, com_type):
    ...
    if com == 'HeatStellarator' or com == 'HeatTokamak':
        power_surplus -= m.demand_fusion[tm, stf, sit, com, com_type]
...
# process output ratio within given boundaries
def def_process_output_ratio_range(m, stf, sit, pro):
    return (m.process_dict['portion_low'][(stf, sit, pro)],
            m.output_pro[stf,sit,pro],
            m.process_dict['portion_up'][(stf, sit, pro)])


# process output with in the given portion
def def_process_output_portion(m, tm, stf, sit, pro, com):
    if com == 'pt_car_pkm':
        if pro == 'pt_car_petrol' or pro == 'pt_car_diesel':
            try:
                return (m.e_pro_out[tm, stf, sit, pro, com] ==
                        m.output_pro[stf, sit, pro] * m.demand_dict[(sit, com)][(stf, tm)])
            except KeyError:
                return pyomo.Constraint.Skip
        else:
            return pyomo.Constraint.Skip
...
def res_global_co2_limit_rule(m, stf):
    if math.isinf(float(m.global_prop_dict['value'][stf, 'CO2 limit'])):
        return pyomo.Constraint.Skip
...
# fusion heat demand from fusion capacity (for all processes, only fusion with factor in inputfile)
def def_continious_losses_rule(m, t, stf, sit, pro):
    if (m.process_dict['cont-losses'][(stf, sit, pro)] == 0):

        for com in m.com_demand:
            if (com == 'HeatStellarator') or (com == 'HeatTokamak'):
                istok = False
                return pyomo.Constraint.Skip
            else:
                try:
                    return (m.demand_fusion[t, stf, sit, com, 'Demand'] == 0)
                except KeyError:
                    return pyomo.Constraint.Skip
    else:

        for com in m.com_demand:
            if (com == 'HeatStellarator' and pro == 'Fusion_Stellarator') or (
                    com == 'HeatTokamak' and pro == 'Fusion_Tokamak'):
                istok = True
                try:
                    return (m.demand_fusion[t, stf, sit, com, 'Demand'] == m.cap_pro[stf, sit, pro] *
                        m.process_dict['cont-losses'][(stf, sit, pro)])
                except KeyError:
                    return pyomo.Constraint.Skip
            else:
                istok = False
