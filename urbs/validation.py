...
def validate_input(data):
    ...
    for (stf, sit, pro) in data['process'].index:
        for com in data['commodity'].index.get_level_values('Commodity'):
            ...
            if ((stf, pro, com) in simplified_pro_com_index and (stf, sit, com) not in simplified_com_index):
    ...
    for index in data['process'].index:
        # Original Statements
        # if not (data['process'].loc[index]['cap-lo'] <= data['process'].loc[index]['cap-up'] and
        # data['process'].fillna(0).loc[index]['inst-cap'] <= data['process'].loc[index]['cap-up']):
        #        raise ValueError('Ensure cap_lo <= cap_up and inst_cap <= cap_up'
        #                         ' for all processes.')

        cond1 = data['process'].loc[index]['cap-lo'] <= data['process'].loc[index]['cap-up']
        cond2 = data['process'].fillna(0).loc[index]['inst-cap'] <= data['process'].loc[index]['cap-up']
        #New Addition
        #print(data)
        print("for data index: ", index)
        if not (cond1 and cond2):
            if not cond1:
                raise ValueError('Ensure cap_lo <= cap_up for all processes.')
            elif not cond2:
                #New Addition
                print(data['process'].fillna(0).loc[index]['inst-cap'])
                print(data['process'].loc[index]['cap-up'])
                raise ValueError('Ensure inst_cap <= cap_up for all processes.')
            else:
                raise ValueError('Ensure cap_lo <= cap_up and inst_cap <= cap_up'
                                 ' for all processes.')
    ...
    for site in data['commodity'].index.levels[0].tolist():
        if site not in data['site'].index.levels[0].tolist():
